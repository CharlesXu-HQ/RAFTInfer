...
- M2 accepts F16 or BF16 GGUF weights only. Q4_K_M, vision, mmproj, MTP,
  speculative decoding, CUDA Graphs, and multi-session scheduling remain out
  of scope.
...
- Qwen3.5 RMSNorm uses `(1 + weight)`, full attention uses Q/K normalization,
  partial RoPE, and a sigmoid output gate, and linear attention retains both
  convolution and FP32 recurrent state.
...
- Operator checks use `abs <= 2e-2` or `rel <= 2e-2` for BF16/F16 CUDA against
  CPU FP32. End-to-end greedy token IDs must match llama.cpp exactly at every
  generated step.
...
- [ ] **Step 3: Implement bounded accounting and ordered-layer parsing**
...
- [ ] **Step 4: Add a checked model payload view**
...
- [ ] **Step 1: Write failing RMSNorm and gated-attention tests**
...
- [ ] **Step 2: Run the reference target and observe RED**
...
- [ ] **Step 3: Implement checked FP32 primitives**
...
- [ ] **Step 4: Add Gated DeltaNet transition vectors**
...
- [ ] **Step 3: Implement immutable layout and preallocated host state**
...
- [ ] **Step 4: Add atomic logical commit and reset tests**
...
- [ ] **Step 1: Write a fixture reader test before adding the fixture**
...
- [ ] **Step 2: Observe RED because the fixture is absent**

- [ ] **Step 3: Export a deterministic small fixture**
...
- [ ] **Step 1: Add C and Rust compile/lifetime tests**
...
- [ ] **Step 2: Observe RED for missing ABI**

- [ ] **Step 3: Implement opaque ownership**
...
- [ ] **Step 4: Run host CTest/Cargo tests and commit**
...
- [ ] **Step 1: Add CUDA tests for F16/BF16 copy and unsupported type**
...
- [ ] **Step 2: Observe RED on the target container**

- [ ] **Step 3: Expose tensor payload spans from the mapped model**

  A payload address is
  `mapped_base + catalog.tensor_data_offset + TensorInfo::offset`; validate the
  full span again before upload.

- [ ] **Step 4: Allocate/upload atomically with RMM**
...
- [ ] **Step 5: Run GPU tests and commit**
...
- Produces project-native kernels for embedding lookup, `(1+weight)` RMSNorm,
...
- [ ] **Step 1: Add parameterized BF16/F16 tests against Task 1**
...
- [ ] **Step 2: Observe RED on RTX 50**

- [ ] **Step 3: Implement correctness-first kernels**
...
- [ ] **Step 1: Add a tiny four-block end-to-end GPU test**
...
- [ ] **Step 3: Implement projection plans**
...
- [ ] **Step 4: Implement ordered hybrid execution**
...
- [ ] **Step 5: Prove no decode allocation**
...
- [ ] **Step 2: Establish llama.cpp golden outputs**
...
- [ ] **Step 3: Run BRT parity under GPU preflight**
...
- [ ] **Step 4: Benchmark only after parity**
...
- [ ] **Step 5: Run all host and GPU checks**
...
- [ ] **Step 6: Update milestone documentation and commit**
