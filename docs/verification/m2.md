...
| CUDA full executor vs CPU reference | `raftinfer_qwen35_executor_test` on RTX 5090 | Pass |
...
The final source was built and tested in `raftinfer-dev:26.06-cuda13` on an NVIDIA
...
`scripts/qwen35-parity.sh` runs llama-server and RAFTInfer sequentially under one
...
raw generated token IDs from llama-server. RAFTInfer emits the same fields through
`raftinfer generate --output-format json`. The first mismatch records its kind,
...
passing. It then measures RAFTInfer and llama.cpp with identical token arrays:
...
- peak RAFTInfer logical GPU bytes allocated through the RMM pool;
- RAFTInfer/llama.cpp throughput ratios with a required floor of 0.8 for both phases.

The RAFTInfer CLI holds one model and session per benchmark arm so loading is excluded
...
| Arm | RAFTInfer prefill | llama.cpp prefill | Ratio | RAFTInfer generation | llama.cpp generation | Ratio |
|---|---:|---:|---:|---:|---:|---:|
...
resource. All RAFTInfer model weights, persistent state, and workspaces use that
...
`peak_memory_status:"measured_by_raftinfer_rmm"`. This is RAFTInfer logical allocation
...
- CUDA build: `<build-root>/cuda`
- Rust release build: `<build-root>/cargo-release`
- parity evidence: `<artifact-root>/qwen35-parity.jsonl`
- benchmark evidence: `<artifact-root>/qwen35-benchmark.jsonl`
