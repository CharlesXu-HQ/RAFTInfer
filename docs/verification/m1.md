...
- Preflight state: no compute applications; free memory 32095 MiB; utilization
  0%; temperature 34 C
- Container image: `raftinfer-dev:26.06-cuda13`
- Image ID:
...
The smoke executable calls the C ABI `raftinfer_engine_run_smoke` path. That path
...
0%; temperature 34 C. Cleanup completed with `active_raftinfer_containers=0`, the
