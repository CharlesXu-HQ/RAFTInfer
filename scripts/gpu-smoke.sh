...
if smoke_output="$(docker run --rm --gpus device=0 \
  --shm-size=1g --ulimit memlock=-1 --ulimit stack=67108864 \
  -v "${repo_root}:/workspace" -w /workspace \
  brt-dev:26.06-cuda13 \
  bash -lc 'cmake -S . -B build/gpu -G Ninja -DBRT_ENABLE_CUDA=ON -DCMAKE_BUILD_TYPE=Release >&2 && cmake --build build/gpu --target brt-smoke >&2 && scripts/gpu-preflight.sh >/dev/null && ./build/gpu/cpp/brt-smoke')"; then
  :
else
  docker_status=$?
  exit "${docker_status}"
fi
printf '%s\n' "${smoke_output}" >"${result_file}"
