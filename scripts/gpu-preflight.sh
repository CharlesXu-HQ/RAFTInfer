...
safety_refusal() {
  echo "RAFTINFER GPU preflight refused: $1" >&2
  exit 23
}
...
is_nonnegative_decimal "${minimum_free_mib}" || safety_refusal "invalid RAFTINFER_MIN_FREE_MIB"
is_nonnegative_decimal "${maximum_utilization}" || safety_refusal "invalid RAFTINFER_MAX_UTILIZATION_PERCENT"
(( 10#${minimum_free_mib} > 0 )) || safety_refusal "invalid RAFTINFER_MIN_FREE_MIB"
(( 10#${maximum_utilization} <= 100 )) || safety_refusal "invalid RAFTINFER_MAX_UTILIZATION_PERCENT"
