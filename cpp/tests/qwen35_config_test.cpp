...
void put_u32(raftinfer::gguf::Catalog &catalog, const std::string &key,
             std::uint32_t value) {
  catalog.metadata.emplace(key, raftinfer::gguf::MetadataValue{value});
}

void put_f32(raftinfer::gguf::Catalog &catalog, const std::string &key, float value) {
  catalog.metadata.emplace(key, raftinfer::gguf::MetadataValue{value});
}

std::vector<raftinfer::gguf::MetadataValue> layer_types_interval4() {
  std::vector<raftinfer::gguf::MetadataValue> values;
  values.reserve(32);
...
void put_layer_types(raftinfer::gguf::Catalog &catalog,
                     std::vector<raftinfer::gguf::MetadataValue> values) {
  catalog.metadata.emplace("qwen35.layer_types",
                           raftinfer::gguf::MetadataValue{raftinfer::gguf::MetadataArray{
                               .element_type = raftinfer::gguf::MetadataType::string,
                               .values = std::move(values),
                           }});
}

void replace_layer_types(raftinfer::gguf::Catalog &catalog,
                         std::vector<raftinfer::gguf::MetadataValue> values) {
...
raftinfer::gguf::Catalog official_qwen35_9b_metadata() {
  raftinfer::gguf::Catalog catalog;
  catalog.metadata.emplace("general.architecture",
                           raftinfer::gguf::MetadataValue{std::string{"qwen35"}});
  ...
  raftinfer::gguf::MetadataArray tokens{
      .element_type = raftinfer::gguf::MetadataType::string,
      .values = {},
  };
  ...
  catalog.metadata.emplace("tokenizer.ggml.tokens",
                           raftinfer::gguf::MetadataValue{std::move(tokens)});
...
template <class Fn> void expect_config_error(Fn &&fn) {
  ...
  try {
    fn();
  } catch (const raftinfer::model::ConfigError &) {
    thrown = true;
  }
  assert(thrown);
}
...
int main() {
  ...
  const auto config = raftinfer::model::derive_qwen35_config(catalog);
  ...
  for (std::size_t index = 0; index < config.blocks.size(); ++index) {
    ...
    assert((config.blocks[index].kind ==
            raftinfer::model::Qwen35BlockKind::full_attention) == expected_full);
  ...
  nextn_catalog.metadata.at("qwen35.block_count") =
      raftinfer::gguf::MetadataValue{std::uint32_t{33}};
  put_u32(nextn_catalog, "qwen35.nextn_predict_layers", 1);
  nextn_catalog.metadata.erase("qwen35.layer_types");
  const auto nextn_config = raftinfer::model::derive_qwen35_config(nextn_catalog);
  ...
  const auto nextn_typed_config =
      raftinfer::model::derive_qwen35_config(nextn_with_main_layer_types);
  ...
  expect_config_error(
      [&] { (void)raftinfer::model::derive_qwen35_config(wrong_layer_count); });
}
