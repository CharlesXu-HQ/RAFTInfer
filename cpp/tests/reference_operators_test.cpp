...
void expect_bf16_bits(float value, std::uint16_t bits) {
  assert(raftinfer::reference::float_to_bf16(value).bits == bits);
}

void expect_bf16_nan(std::uint32_t f32_bits) {
  const auto bf16 = raftinfer::reference::float_to_bf16(float_from_bits(f32_bits));
  assert((bf16.bits & 0x7F80U) == 0x7F80U);
  assert((bf16.bits & 0x007FU) != 0);
}

std::vector<float> scalar_rope(std::span<const float> input, raftinfer::reference::RopeShape shape) {
...
int main() {
  using raftinfer::reference::AddShape;
  using raftinfer::reference::EmbeddingShape;
  using raftinfer::reference::LinearShape;
  using raftinfer::reference::RmsNormShape;
  using raftinfer::reference::RopeShape;
  using raftinfer::reference::SoftmaxShape;
  using raftinfer::reference::SwiGluShape;

  {
    const std::array<float, 4> lhs{1.0F, -2.0F, 0.5F, 4.0F};
    const std::array<float, 4> rhs{3.0F, 2.0F, -0.5F, -1.0F};
    std::array<float, 4> out{};
    raftinfer::reference::add(lhs, rhs, out, AddShape{4});
...

  {
    ...
    raftinfer::reference::embedding(tokens, table, out, EmbeddingShape{3, 4, 3});
...

  {
    ...
    assert(raftinfer::reference::argmax(logits) == 1);
  }

  {
    ...
    std::array<float, 4> out{};
    raftinfer::reference::rms_norm(x, weight, out, RmsNormShape{2, 2}, 0.0F);
...

  {
    ...
    raftinfer::reference::rope(x, zero, RopeShape{1, 1, 8, 4, 0, 10000.0F, true});
    expect_vector_near(zero, x);
    raftinfer::reference::rope(x, one, RopeShape{1, 1, 8, 4, 1, 10000.0F, true});
...

  {
    ...
    raftinfer::reference::rope(x, out, RopeShape{1, 1, 6, 4, 3, 100.0F, false});
...

  {
    ...
    std::array<raftinfer::reference::bf16_t, 4> x{};
    std::array<raftinfer::reference::bf16_t, 4> w{};
    std::transform(x_f32.begin(), x_f32.end(), x.begin(), raftinfer::reference::float_to_bf16);
    std::transform(w_f32.begin(), w_f32.end(), w.begin(), raftinfer::reference::float_to_bf16);
    std::array<float, 4> out{};
    raftinfer::reference::bf16_linear(x, w, out, LinearShape{2, 2, 2});
...

  {
    ...
    raftinfer::reference::softmax(logits, out, SoftmaxShape{1, 3});
...

  {
    ...
    std::array<float, 6> out{};
    raftinfer::reference::swiglu(gate, up, out, SwiGluShape{6});
...

  {
    ...
    for (int trial = 0; trial < 32; ++trial) {
      ...
      raftinfer::reference::add(x, y, out, AddShape{x.size()});
      for (std::size_t i = 0; i < x.size(); ++i) {
        expect_near(out[i], x[i] + y[i]);
      }

      raftinfer::reference::swiglu(x, y, out, SwiGluShape{x.size()});
      for (std::size_t i = 0; i < x.size(); ++i) {
        expect_near(out[i], silu(x[i]) * y[i]);
      }

      raftinfer::reference::softmax(x, out, SoftmaxShape{8, 8});
      ...
      raftinfer::reference::rms_norm(x, weight, out, RmsNormShape{8, 8}, 1.0e-5F);
      for (std::size_t row = 0; row < 8; ++row) {
      ...
      raftinfer::reference::rope(x, out, RopeShape{2, 2, 16, 8, static_cast<std::size_t>(trial + 1),
                                             10000.0F, (trial % 2) == 0});
      ...
      std::array<raftinfer::reference::bf16_t, 15> linear_input{};
      std::array<raftinfer::reference::bf16_t, 20> linear_weight{};
      ...
      std::transform(linear_input_f32.begin(), linear_input_f32.end(), linear_input.begin(),
                     raftinfer::reference::float_to_bf16);
      std::transform(linear_weight_f32.begin(), linear_weight_f32.end(), linear_weight.begin(),
                     raftinfer::reference::float_to_bf16);
      std::array<float, 12> linear_out{};
      raftinfer::reference::bf16_linear(linear_input, linear_weight, linear_out, LinearShape{3, 5, 4});
      for (std::size_t row = 0; row < 3; ++row) {
        for (std::size_t col = 0; col < 4; ++col) {
          float sum = 0.0F;
          for (std::size_t k = 0; k < 5; ++k) {
            sum += raftinfer::reference::bf16_to_float(linear_input[row * 5 + k]) *
                   raftinfer::reference::bf16_to_float(linear_weight[k * 4 + col]);
          }
      ...
      raftinfer::reference::embedding(tokens, table, embedding_out, EmbeddingShape{6, 5, 7});
      ...
      arg_values[10] = 9.0F;
      arg_values[20] = 9.0F;
      assert(raftinfer::reference::argmax(arg_values) == 10);
    }
  }

  {
    ...
    raftinfer::reference::add(std::span<const float>{}, std::span<const float>{}, std::span<float>{},
                        AddShape{0});
    raftinfer::reference::swiglu(std::span<const float>{}, std::span<const float>{}, std::span<float>{},
                           SwiGluShape{0});
    raftinfer::reference::bf16_linear(std::span<const raftinfer::reference::bf16_t>{},
                                std::array<raftinfer::reference::bf16_t, 6>{}, std::span<float>{},
                                LinearShape{0, 3, 2});
    raftinfer::reference::embedding(std::span<const std::int32_t>{}, std::array<float, 4>{},
                              std::span<float>{}, EmbeddingShape{0, 2, 2});
    raftinfer::reference::rope(std::span<const float>{}, std::span<float>{},
                         RopeShape{0, 1, 4, 4, 0, 10000.0F, true});
    raftinfer::reference::softmax(std::span<const float>{}, std::span<float>{}, SoftmaxShape{0, 3});

    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::add(std::array<float, 3>{}, std::array<float, 4>{}, out, AddShape{4});
    });
    expect_vector_equal(out, sentinel4);

    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::rms_norm(std::array<float, 4>{}, std::array<float, 2>{}, out,
                               RmsNormShape{2, 2}, -1.0e-5F);
    });
    expect_vector_equal(out, sentinel4);
    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::rms_norm(std::span<const float>{}, std::span<const float>{}, out,
                               RmsNormShape{max_size, 2}, 0.0F);
    });
    expect_vector_equal(out, sentinel4);
    expect_invalid_argument([&] {
      raftinfer::reference::rms_norm(std::span<const float>{}, std::span<const float>{},
                               std::span<float>{}, RmsNormShape{1, 0}, 0.0F);
    });
    raftinfer::reference::rms_norm(std::span<const float>{}, std::array<float, 3>{},
                             std::span<float>{}, RmsNormShape{0, 3}, 1.0e-5F);

    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::rope(std::array<float, 4>{}, out, RopeShape{1, 1, 4, 6, 0, 10000.0F, false});
    });
    expect_vector_equal(out, sentinel4);
    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::rope(std::array<float, 4>{}, out, RopeShape{1, 1, 4, 4, 0, 0.0F, false});
    });
    expect_vector_equal(out, sentinel4);
    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::rope(std::array<float, 4>{}, out, RopeShape{1, 1, 4, 3, 0, 10000.0F, false});
    });
    expect_vector_equal(out, sentinel4);
    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::rope(std::span<const float>{}, out,
    ...
    expect_invalid_argument([&] {
      raftinfer::reference::bf16_linear(std::span<const raftinfer::reference::bf16_t>{},
                                  std::span<const raftinfer::reference::bf16_t>{}, out,
    ...
    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::embedding(std::array<std::int32_t, 2>{0, -1}, std::array<float, 4>{}, out,
                                EmbeddingShape{2, 2, 2});
    });
    expect_vector_equal(out, sentinel4);
    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::embedding(std::array<std::int32_t, 2>{0, 2}, std::array<float, 4>{}, out,
                                EmbeddingShape{2, 2, 2});
    });
    expect_vector_equal(out, sentinel4);
    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::embedding(std::span<const std::int32_t>{}, std::span<const float>{}, out,
    ...
    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::softmax(std::span<const float>{}, out, SoftmaxShape{1, 0});
    });
    expect_vector_equal(out, sentinel4);
    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::softmax(std::span<const float>{}, out, SoftmaxShape{max_size, 2});
    });
    expect_vector_equal(out, sentinel4);

    out = sentinel4;
    expect_invalid_argument([&] {
      raftinfer::reference::swiglu(std::array<float, 3>{}, std::array<float, 4>{}, out, SwiGluShape{4});
    });
    expect_vector_equal(out, sentinel4);

    expect_invalid_argument([&] {
      static_cast<void>(raftinfer::reference::argmax(std::span<const float>{}));
    });
  }
}
