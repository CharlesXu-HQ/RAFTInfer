...
std::vector<float> sequence(std::size_t elements, float scale, float bias) {
  ...
  for (std::size_t i = 0; i < values.size(); ++i) {
    values[i] =
        bias + scale * static_cast<float>(static_cast<int>((i * 13) % 17) - 8);
  }
  return values;
}

template <typename T>
void assert_matches(std::span<const T> actual,
...
template <typename T> void run_dtype_tests(raftinfer::ExecutionContext &context) {
  for (const std::size_t rows :
       {std::size_t{1}, std::size_t{2}, std::size_t{4}, std::size_t{17}}) {
    {
      ...
      const std::vector<std::int32_t> tokens = [&] {
        ...
        for (std::size_t i = 0; i < rows; ++i)
          ids[i] = static_cast<int>(i % 5);
        return ids;
      }();
      const auto table_f32 =
          sequence(vocab_size * embedding_dim, 0.031F, -0.2F);
...

    {
      ...
      const std::vector<float> unit_weight_f32(cols, 1.0F);
      const auto input = encode<T>(input_f32);
      const auto weight = encode<T>(unit_weight_f32);
...
void run_f32_weight_tests(raftinfer::ExecutionContext &context) {
  ...
  const auto input_f32 = sequence(rows * cols, 0.047F, 0.1F);
  const auto input = encode<T>(input_f32);
  const auto quantized_input = decode<T>(input);
  const auto weight = sequence(cols, 0.0037F, 0.913F);
  const auto device_input = upload(context, std::span{input});
  const auto device_weight = upload(context, std::span{weight});
  DeviceBuffer device_output{context, input.size() * sizeof(T)};
  ...
  for (std::size_t row = 0; row < tokens.size(); ++row) {
    for (std::size_t col = 0; col < cols; ++col) {
      assert(
          ...
          quantized_input[static_cast<std::size_t>(tokens[row]) * cols + col]);
    }
  }
...
int main() {
  ...
  run_dtype_tests<float>(context);
  run_f32_weight_tests<__half>(context);
  run_f32_weight_tests<__nv_bfloat16>(context);
